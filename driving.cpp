...
EndCondition::EndCondition(Luminous* luminous, Odometry* odometry)
    : luminous_(luminous), odometry_(odometry),
...
void EndCondition::SetParam(End end_type, Color end_color, float end_threshold) {
  ...
  if (end_type_ == kDistanceEnd) {
    ref_distance_ = odometry_->distance_;
  } else if (end_type_ == kThetaEnd) {
    ref_theta_ = odometry_->theta;
  }
}
...
bool EndCondition::IsSatisfied() {
  ...
  switch (end_type_) {
    ...
    case kDistanceEnd:
      if (end_threshold_ > 0 && odometry_->distance_ - ref_distance_ > end_threshold_)
        end_state_ = true;
      else if (end_threshold_ < 0 && odometry_->distance_ - ref_distance_ < end_threshold_)
        end_state_ = true;
      break;

    case kThetaEnd:
      if (end_threshold_ > 0 && odometry_->theta - ref_theta_ > end_threshold_)
        end_state_ = true;
      else if (end_threshold_ < 0 && odometry_->theta - ref_theta_ < end_threshold_)
        end_state_ = true;
      break;
